...
```
The main dataset is of dimensions **`r dim(covid)`**, whereas the mainly used date for  **Mexico** alone is **`r dim(covid_mex)`**.

## Analysis & Findings

### The Trend of Numbers
...
case$date <- with(case, sprintf("%d-%02d", year, month))

case <- case %>% select(1,2,3,4) %>% 
  group_by(date) 
...
case %>% 
  summarise(new_cases = spk_chr(new_cases,
            ...
            new_deaths = spk_chr(new_deaths,
                                ...
                                highlightLineColor = 'orange', 
                                highlightSpotColor = 'orange'),
            new_tests = spk_chr(new_tests,
                                type = "line",
                                highlightLineColor = 'orange', 
                                highlightSpotColor = 'orange')) %>%
  datatable(escape = FALSE,
            rownames = FALSE,
            colnames = c("Date", "New Cases", "New Death", "New Tests"),
            class = list(display = c("striped", "hover")),
    ...
    spk_add_deps() 
```
This table includes the date, new cases, new death people and new tests people by month since January 2020 to August 2021.It can search by month or year as wanted in the table' which is really convenient for reading the table.

The vacancy in the number of new deaths in January and February 2020 implies that there were not many deaths caused by the COVID virus, but this may be because the COVID cases only appeared and grew rapidly at the end of February. In contrast, new cases fluctuated greatly from March to September 2020. new cases and deaths decreased to a low point in October 2020 but continued to fluctuate again in November. The number of deaths fluctuated downward until March 2021, but the number of new cases still fluctuated significantly until the latest.


```{r totaldeath, fig.cap="The Growth of Carform Cases and Death Number in Mexico"}
options(scipen=200)
covid_mex%>% 
...
wdeathrate <- covid%>% 
  ...
  drop_na() 
```
The world average COVID death rate is `r round(mean(wdeathrate$death_rate)*100, digits = 2)` and the average COVID death rate of Mexico is `r round(mean(deathrate$death_rate), digits = 2)`, Which is `r round(mean(deathrate$death_rate)/mean(wdeathrate$death_rate), digits = 2)` times of the world average.
